...
int main(int argc, char *argv[]){
  ...
  if(!infile1){
    ...
    // Component[i] is the id of the ith vertex
    fill_n(Component, V, -1);
    // every node has id -1 at the begining
...
{  
    int NCC_BFS  = 0;
    int current = -1;
    fill_n(Component, V, -1); // initialize component
    ...
    for (int kk = 0; kk < V; kk++){
        if (Component[kk] < 0){
            ...
            while (!Q.empty()) {
              current = Q.front();
              Component[current]= NCC_BFS;
              Q.pop();
              for (int n = FirstVertex[current]; n < FirstVertex[current+1]; n++) {
                if (!visited[EdgeList[n]]) {
                  visited[EdgeList[n]] = true;
                  Q.push(EdgeList[n]);
                }
              }
            }
...
void DFS(int v, int *FirstVertex, int V, int *EdgeList,int E, int *Component,int NCC){
    ...
    Component[v] = NCC;
    for (int n = FirstVertex[v]; n < FirstVertex[v+1]; n++) {
      if (Component[EdgeList[n]] < 0) {
        DFS(EdgeList[n], FirstVertex, V, EdgeList, E, Component, NCC);
      }
    }
